...
public class ShotPoint {
    public final double camAngle;
    public final double hoodDeg;
    public final double rpm;

    public ShotPoint(double camAngle, double hoodDeg, double rpm) {
    ...
    public ShotPoint interpolate(double camAngle) {
        ...
        for (int i = 0; i < LUT.length - 1; i++) {
            ...
            // NOTE the comparison signs
            if (camAngle <= a.camAngle && camAngle >= b.camAngle) {

                double t = (camAngle - a.camAngle) /
                        (b.camAngle - a.camAngle);
